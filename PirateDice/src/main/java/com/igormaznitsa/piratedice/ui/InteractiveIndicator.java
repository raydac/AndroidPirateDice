...
public class InteractiveIndicator extends SurfaceView implements SurfaceHolder.Callback, Runnable, View.OnLongClickListener, Model.ModelListener {
        ...
        synchronized (gfxLock) {
            try {
                switch (Model.getInstance().getType()) {
                    case STANDARD: {
                        vertushka = SVGParser.getSVGFromAsset(manager, "svg/vertushka.svg");
                    }
                    break;
                    case PROFESSIONAL: {
                        vertushka = SVGParser.getSVGFromAsset(manager, "svg/vertushka_pro.svg");
                    }
                    break;
                }
                resize();
    ...
    public void onPause(Model m, boolean pause) {
        try{
            if (pause) this.theThread.suspend();
            else this.theThread.resume();
        }catch(Exception ex){
            ex.printStackTrace();
        }
    }
